...
void BugReportingHostApiSetup(id<FlutterBinaryMessenger> binaryMessenger, NSObject<BugReportingHostApi> *api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.instabug_flutter.BugReportingHostApi.addUserConsents"
        binaryMessenger:binaryMessenger
        codec:BugReportingHostApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(addUserConsentsKey:description:mandatory:checked:actionType:error:)], @"BugReportingHostApi api (%@) doesn't respond to @selector(addUserConsentsKey:description:mandatory:checked:actionType:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        NSString *arg_key = GetNullableObjectAtIndex(args, 0);
        NSString *arg_description = GetNullableObjectAtIndex(args, 1);
        NSNumber *arg_mandatory = GetNullableObjectAtIndex(args, 2);
        NSNumber *arg_checked = GetNullableObjectAtIndex(args, 3);
        NSString *arg_actionType = GetNullableObjectAtIndex(args, 4);
        FlutterError *error;
        [api addUserConsentsKey:arg_key description:arg_description mandatory:arg_mandatory checked:arg_checked actionType:arg_actionType error:&error];
        callback(wrapResult(nil, error));
      }];
    } else {
      [channel setMessageHandler:nil];
